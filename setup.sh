...
setup_zram() {
    ...
    # 1) Мягко остановить сервис и отключить все активные zram swap
    ...
    # 2) Гарантированно записать конфиг zramswap
...
EOF
    ...
    # 3) Попытка запустить системный сервис
    ...
    # 4) Ручная настройка zram (fallback)
    modprobe -r zram 2>/dev/null || true
    modprobe zram num_devices=1 2>/dev/null || true

    if [[ -b /dev/zram0 ]]; then
        # Настройка алгоритма и размера
        ...
        log "WARN" "zram kernel module not available on this kernel"
    fi

    # 5) Не фейлим установку — продолжаем, отметим в валидации
...
validate_installation() {
    ...
    # Проверка zram - улучшенная версия
    if systemctl is-enabled --quiet zramswap; then
        log "INFO" "✅ zramswap service is enabled"
        ((checks_passed++))
    else
        log "WARN" "⚠️  zramswap service is not enabled"
        ((validation_failed++))
    fi
    ((total_checks++))

    if systemctl is-active --quiet zramswap || swapon --show | grep -q "/dev/zram0"; then
        log "INFO" "✅ zram is active (service or manual /dev/zram0)"
        ((checks_passed++))
    else
        log "WARN" "⚠️  zram not active (may need reboot or manual setup)"
        # Не считаем критом — просто предупреждение, чтобы установка не падала
    fi
    ((total_checks++))
